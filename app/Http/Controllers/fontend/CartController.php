...
class CartController extends Controller
{
  ...
  public function index()
  {
    ...
    return view('fontend.carts',compact('cart','coupon'));
  }
  ...
  public function cuponcode_apply()
  {
    ...
    if(!empty($coupon))
    {
      $coupon_details =Coupon::where('user_id',$userdata->id)->where('status','apply')->first();
      if($coupon->status != 'expired' && $coupon->status !='apply')
       {
         $coupon->status = 'apply';
         $coupon->save();
         return response()->json(['stat'=>true,'message'=>"coupon code applied successfully",'data'=>$coupon_details]);
       }
       else if($coupon->status == 'apply')
       {
          return response()->json(['stat'=>false,'message'=>"You have already applied a coupon code",'data'=>[]]);
       }
       else
       {
          return response()->json(['stat'=>true,'message'=>"coupon code applied successfully",'data'=>[]]);
       }
    }
    else
    {
      return response()->json(['stat'=>true,'message'=>"coupon code is invalid",'data'=>[]]);
    }
  }
}
